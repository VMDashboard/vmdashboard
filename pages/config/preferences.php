...
if (isset($_SESSION['themeColorChange'])) {
  ...
  $sql = "SELECT name FROM openvm_config WHERE name = 'theme_color' AND userid = '$userid';";
  $result = $conn->query($sql);

  if (mysqli_num_rows($result) == 0 ) {
    $sql = "INSERT INTO openvm_config (name, value, userid) VALUES ('theme_color', '$themeColorChange', '$userid');";
    $result = $conn->query($sql);
  } else {
    $sql = "UPDATE openvm_config SET value = '$themeColorChange' WHERE name = 'theme_color' AND userid = '$userid';";
    $result = $conn->query($sql);
  }
